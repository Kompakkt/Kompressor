...
## Prepare LAStools ##
WORKDIR /opt/
RUN apt-get update && apt-get -y install git build-essential cmake libjpeg62 libpng-dev libtiff-dev libjpeg-dev libz-dev libproj-dev liblzma-dev libjbig-dev libzstd-dev libgeotiff-dev libwebp-dev liblzma-dev libsqlite3-dev && \
...
FROM ghcr.io/kompakkt/gsbox-release AS gsbox-builder
...
COPY --from=lastools-builder /opt/LAStools /opt/LAStools
